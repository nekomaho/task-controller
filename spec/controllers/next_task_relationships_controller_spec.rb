...
RSpec.describe NextTaskRelationshipsController, type: :controller do
  describe 'POST #creta' do
    ...
    context 'when access no login user' do
      it 'redirect to login page' do
        expect(
          post(:create, params: { prev_task: input.id, next_task: { id: next_task.id } })
    ...
    context 'another project task' do
      ...
      it 'redirect to top page' do
        expect(
          post(:create, params: { prev_task: input.id,
                                  next_task: { id: another_project_task.id } })
  ...
  describe 'DELETE #destroy' do
    ...
    context 'when access no login user' do
      it 'redirect to login page' do
        expect(
          delete(:destroy, params: { id: input.id, delete_id: next_task.id })
        ).to redirect_to(new_user_session_path)
      end
    end
    context 'when access login user' do
